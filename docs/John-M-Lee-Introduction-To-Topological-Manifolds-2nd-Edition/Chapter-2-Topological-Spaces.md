...
Finally we must show that $\text{Int}A \cup \partial A \subseteq \overline{A}$. Let $x \in \text{Int}A \cup \partial A$. NTS $x \in \overline{A}$.
Let $B$ be a neighborhood of $x$. By (d) NTS $B$ contains a point of $A$. We will handle the two cases separately: $x \in \text{Int}A$ and $x \in \partial A$.
